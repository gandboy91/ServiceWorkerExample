...
const LoginForm = React.memo(({ errors, loginRequest, isProcessing = false }) => {
    const { email, password, changeEmailHandler, changePasswordHandler } = useAuthorizationForm()
    const handleLoginRequest = useCallback(() => loginRequest(email, password), [email, password])
            ...
            </div>
            {
                isProcessing
                    ...
                    : <div className='card-body'>
                        ...
                        />
                        <button className='btn btn-secondary btn-block' onClick={handleLoginRequest}>
                            Log in
                        </button>
