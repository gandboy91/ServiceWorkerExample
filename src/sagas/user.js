...
import {
    ...
    LOGOUT_REQUEST,
    logoutRequest,
    logoutSuccess, REGISTER_REQUEST,
    setUser,
    USER_REQUEST
} from "../actions/user"
...
function* registerWorker({ payload: { name, email, password } }) {
    try {
        yield put(startFetching)
        const { token, user: {created_at, updated_at, ...user} } = yield call(register, { email, password, name })
        saveToStorage(TOKEN_STORAGE_KEY, token || '')
...
function* getUserWorker() {
    // try {
    //     const { user: { id, email, name, role } } = yield call(getUserByToken)
    //
    //     yield put(setUser({ id, email, name, role }))
    // } catch (error) {
    //     console.warn(error)
    // } finally {
    //
...
function* getCurrentUser() {
    const { user: {created_at, updated_at, ...user} } = yield call(getUserByToken)
    saveToStorage(USER_STORAGE_KEY, user || {})
    yield put(setUser(user))
}
...
function* userWatcher() {
    yield all([
        ...
        takeLatest(USER_REQUEST, getUserWorker)
    ])
}
